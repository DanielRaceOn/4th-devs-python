...
# tracing/ -> core/ -> src/ -> 03_01_observability/
...
def _load_state() -> dict[str, Any]:
    ...
    Returns:
        Dict mapping prompt name to ``{"hash": str, "version": int}``.
    """
...
def _save_state(state: dict[str, Any]) -> None:
    ...
    Args:
        state: Dict mapping prompt name to ``{"hash": str, "version": int}``.
    """
    try:
...
async def _collect_prompt_sources() -> list[dict[str, Any]]:
    ...
    # Use relative import since this file lives inside the src package.
...
async def sync_prompts() -> None:
    ...
    On error, a warning is printed but execution continues (non-fatal).
    """
    ...
    for source in sources:
        ...
        if existing.get("hash") == new_hash:
            # Content unchanged — restore cache from stored version
            stored_version = existing.get("version", 0)
            _prompt_cache[name] = PromptRef(
                name=name, version=stored_version, is_fallback=False
            )
        ...
        try:
            ...
            _prompt_cache[name] = PromptRef(
                name=name, version=version, is_fallback=False
            )
            ...
            # Use fallback if we have a prior version
            stored_version = existing.get("version", 0)
            if stored_version:
                _prompt_cache[name] = PromptRef(
                    name=name, version=stored_version, is_fallback=True
                )
...
def get_prompt_ref_by_name(name: str) -> Optional[PromptRef]:
    ...
    Returns:
        The cached ``PromptRef``, or ``None`` if the prompt has not been synced.
    """
